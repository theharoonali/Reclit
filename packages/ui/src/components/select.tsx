...
import { menuLabel, menuSeparator, menuSurface } from "../styles/menu";
import { cn } from "../utils";

/**
 * The shared select — shadcn's, on Radix.
 *
 * `SelectTrigger` is deliberately `Input`'s twin: same height, radius, border
 * token and focus ring, so a form reads as one control repeated rather than
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    className={cn(
      fieldSingleLine,
      "items-center justify-between gap-inline text-foreground",
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-body outline-none",

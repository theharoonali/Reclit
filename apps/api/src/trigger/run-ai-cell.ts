...
// One AI cell, end to end. The API has already recorded the run as `pending`
// with `result.input`; this task moves it `running`, asks the model, and
// `complete`s it (the service writes `result.output` into the cell before
// flipping the status) or `fail`s it with the reason. Every transition fires
// the Postgres trigger, so the sheet sees it live.
//
...
export const runAiCellTask = schemaTask({
  ...
  run: async ({ runId, input }) => {
    ...
    try {
      ...
      return await runAiService.complete(runId, {
